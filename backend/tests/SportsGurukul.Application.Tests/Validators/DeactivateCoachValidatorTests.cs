...
public class DeactivateCoachValidatorTests
{
    private readonly DeactivateCoachValidator _validator = new();

    [Fact]
    public async Task ValidCommand_ShouldNotHaveValidationErrors()
    {
        var command = new DeactivateCoachCommand { CoachId = Guid.NewGuid() };
    ...
    {
        var command = new DeactivateCoachCommand { CoachId = Guid.Empty };
