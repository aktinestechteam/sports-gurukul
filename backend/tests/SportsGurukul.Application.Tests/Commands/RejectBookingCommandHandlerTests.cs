...
public class RejectBookingCommandHandlerTests
{
    ...
    private readonly Mock<ILogger<RejectBookingCommandHandler>> _loggerMock = TestMocks.CreateLogger<RejectBookingCommandHandler>();
    ...
    public async Task Handle_BookingNotFound_ReturnsFailure()
    {
        ...
        var result = await _handler.Handle(new RejectBookingCommand { BookingId = Guid.NewGuid() }, CancellationToken.None);
    ...
    public async Task Handle_NotPending_ReturnsFailure()
    {
        ...
        var result = await _handler.Handle(new RejectBookingCommand { BookingId = booking.Id }, CancellationToken.None);

        result.IsSuccess.Should().BeFalse();
        result.Error.Should().Be("Only pending bookings can be rejected.");
    }

    [Fact]
    public async Task Handle_PendingBooking_RejectsSuccessfully()
    {
        ...
        var result = await _handler.Handle(new RejectBookingCommand
        {
        ...
        booking.ApprovalStatus.Should().Be(BookingApprovalStatus.Rejected);
    }
}
