...
public interface IAcademyRepository : IRepository<Academy>
{
    ...
    Task<Academy?> GetByIdIncludingDeletedAsync(Guid id, CancellationToken cancellationToken = default);
    ...
    Task<Academy?> GetByOwnerUserIdAsync(Guid userId, CancellationToken cancellationToken = default);
