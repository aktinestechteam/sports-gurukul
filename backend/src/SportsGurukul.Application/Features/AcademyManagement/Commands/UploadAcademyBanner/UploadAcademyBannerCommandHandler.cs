...
public class UploadAcademyBannerCommandHandler : IRequestHandler<UploadAcademyBannerCommand, Result<AcademyDto>>
{
    private readonly IAcademyRepository _academyRepository;
    private readonly IFileStorageService _fileStorageService;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILogger<UploadAcademyBannerCommandHandler> _logger;

    public UploadAcademyBannerCommandHandler(
        IAcademyRepository academyRepository,
        IFileStorageService fileStorageService,
        IUnitOfWork unitOfWork,
        ILogger<UploadAcademyBannerCommandHandler> logger)
    {
    ...
    public async Task<Result<AcademyDto>> Handle(UploadAcademyBannerCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Uploading banner for academy: {AcademyId}", request.AcademyId);
        ...
        academy.BannerUrl = storageResult.PublicUrl ?? storageResult.StoragePath;
        academy.UpdatedAt = DateTime.UtcNow;
        _academyRepository.Update(academy);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        _logger.LogInformation("Banner uploaded for academy: {AcademyId}, Url: {Url}", request.AcademyId, academy.BannerUrl);
