...
public class RecordRecentSearchCommandHandler : IRequestHandler<RecordRecentSearchCommand, Result<Unit>>
{
    private readonly IRecentSearchRepository _repository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILogger<RecordRecentSearchCommandHandler> _logger;

    private const int MaxRecentSearches = 20;

    public RecordRecentSearchCommandHandler(
        IRecentSearchRepository repository,
        IUnitOfWork unitOfWork,
        ILogger<RecordRecentSearchCommandHandler> logger)
    {
    ...
    public async Task<Result<Unit>> Handle(RecordRecentSearchCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Recording recent search for user: {UserId}", request.UserId);
