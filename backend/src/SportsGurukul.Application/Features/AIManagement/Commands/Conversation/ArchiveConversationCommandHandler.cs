...
public class ArchiveConversationCommandHandler : IRequestHandler<ArchiveConversationCommand, Result<ConversationDto>>
{
    ...
    public ArchiveConversationCommandHandler(IConversationService conversationService, IUnitOfWork unitOfWork)
    {
        _conversationService = conversationService;
        _unitOfWork = unitOfWork;
    }

    public async Task<Result<ConversationDto>> Handle(ArchiveConversationCommand request, CancellationToken cancellationToken)
    {
        var result = await _conversationService.ArchiveAsync(request.Id, cancellationToken);
        if (!result.IsSuccess)
