...
public class SummarizeConversationCommandHandler : IRequestHandler<SummarizeConversationCommand, Result<ConversationDto>>
{
    ...
    public SummarizeConversationCommandHandler(IConversationService conversationService, IUnitOfWork unitOfWork)
    {
    ...
    {
        var result = await _conversationService.SummarizeAsync(request.ConversationId, cancellationToken);
        if (!result.IsSuccess)
