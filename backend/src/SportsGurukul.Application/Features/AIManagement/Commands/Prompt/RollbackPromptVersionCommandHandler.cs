...
public class RollbackPromptVersionCommandHandler : IRequestHandler<RollbackPromptVersionCommand, Result<PromptTemplateDto>>
{
    ...
    public RollbackPromptVersionCommandHandler(IPromptService promptService, IUnitOfWork unitOfWork)
    {
    ...
    {
        var result = await _promptService.RollbackAsync(request.Id, request.VersionNumber, cancellationToken);
        if (!result.IsSuccess)
