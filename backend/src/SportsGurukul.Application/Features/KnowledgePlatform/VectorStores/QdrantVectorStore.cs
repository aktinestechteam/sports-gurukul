...
public class QdrantVectorStore : BaseVectorStore
{
    public override VectorStoreType StoreType => VectorStoreType.Qdrant;
    public override string StoreName => "Qdrant";
    ...
    {
        InMemoryIndices[indexName] = new InMemoryIndex
        {
            Name = indexName,
    ...
    public override Task DeleteIndexAsync(string indexName, CancellationToken cancellationToken = default)
    {
        InMemoryIndices.Remove(indexName);
        return Task.CompletedTask;
    }

    public override Task<bool> IndexExistsAsync(string indexName, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(InMemoryIndices.ContainsKey(indexName));
    }
    ...
    {
        if (InMemoryIndices.TryGetValue(indexName, out var index))
            index.Entries.RemoveAll(e => chunkIds.Contains(e.ChunkId));
        return Task.CompletedTask;
    }
    ...
    {
        if (InMemoryIndices.TryGetValue(indexName, out var index))
            index.Entries.RemoveAll(e => e.DocumentId == documentId);
        return Task.CompletedTask;
    }
    ...
    public override Task<List<SearchResult>> SemanticSearchAsync(string indexName, float[] queryVector, int topK = 10, double? scoreThreshold = null, Dictionary<string, string>? metadataFilters = null, CancellationToken cancellationToken = default)
    {
        ...
        var filtered = metadataFilters != null
            ? index.Entries.Where(e => e.Metadata != null && metadataFilters.All(f => e.Metadata.TryGetValue(f.Key, out var v) && v == f.Value)).ToList()
            : index.Entries;

        var scored = filtered
            .Select(e => (Score: CosineSimilarity(queryVector, e.Vector), e))
            .ToList();

        var results = ApplyScoreThreshold(scored.Select(s => (s.Score, new SearchResult(
            DocumentId: s.e.DocumentId,
            ChunkId: s.e.ChunkId,
            Content: s.e.Content,
            Score: s.Score,
            DocumentName: s.e.DocumentName,
            Format: s.e.Format,
            PageNumber: s.e.PageNumber,
            Section: s.e.Section,
            Metadata: s.e.Metadata,
            Citation: new Citation(s.e.DocumentName, s.e.Section, s.e.PageNumber, s.e.ChunkId, s.Score, null, s.e.Content[..Math.Min(200, s.e.Content.Length)])
        ))).ToList(), scoreThreshold);
    ...
    public override Task<List<SearchResult>> HybridSearchAsync(string indexName, float[] queryVector, string keywordQuery, int topK = 10, double? scoreThreshold = null, Dictionary<string, string>? metadataFilters = null, double semanticWeight = 0.7, CancellationToken cancellationToken = default)
    {
        ...
        var keywordTerms = keywordQuery.ToLower().Split(' ', StringSplitOptions.RemoveEmptyEntries);
        var filtered = metadataFilters != null
            ? index.Entries.Where(e => e.Metadata != null && metadataFilters.All(f => e.Metadata.TryGetValue(f.Key, out var v) && v == f.Value)).ToList()
            : index.Entries;
        ...
        {
            var semanticScore = CosineSimilarity(queryVector, e.Vector);
            var keywordScore = keywordTerms.Any()
                ? keywordTerms.Count(t => e.Content.ToLower().Contains(t)) / (double)keywordTerms.Length
                : 0;

            return (Score: semanticScore * semanticWeight + keywordScore * (1 - semanticWeight), e);
        }).ToList();
    ...
    public override Task<List<SearchResult>> KeywordSearchAsync(string indexName, string query, int topK = 10, Dictionary<string, string>? metadataFilters = null, CancellationToken cancellationToken = default)
    {
        ...
        var scored = filtered
            .Select(e => (Score: terms.Any() ? terms.Count(t => e.Content.ToLower().Contains(t)) / (double)terms.Length : 0, e))
            .Where(s => s.Score > 0)
    ...
    {
        return Task.FromResult((long)(InMemoryIndices.TryGetValue(indexName, out var index) ? index.Entries.Count : 0));
    }
    ...
    public override Task<Dictionary<string, object>> GetIndexStatsAsync(string indexName, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(new Dictionary<string, object>
        {
            ...
            ["vector_count"] = index.Entries.Count,
            ["document_count"] = index.Entries.Select(e => e.DocumentId).Distinct().Count()
        });
    }
}
