...
public class AzureAISearchVectorStore : BaseVectorStore
{
    public override VectorStoreType StoreType => VectorStoreType.AzureAISearch;
    public override string StoreName => "Azure AI Search";
    ...
    public override Task<List<SearchResult>> HybridSearchAsync(string indexName, float[] queryVector, string keywordQuery, int topK = 10, double? scoreThreshold = null, Dictionary<string, string>? metadataFilters = null, double semanticWeight = 0.7, CancellationToken cancellationToken = default)
    {
        ...
        var scored = filtered.Select(e =>
        {
            ...
            var keywordScore = terms.Any() ? terms.Count(t => e.Content.ToLower().Contains(t)) / (double)terms.Length : 0;
