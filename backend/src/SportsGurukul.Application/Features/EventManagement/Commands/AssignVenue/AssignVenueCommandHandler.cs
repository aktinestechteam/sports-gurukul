...
public class AssignVenueCommandHandler : IRequestHandler<AssignVenueCommand, Result<EventSessionDto>>
{
    private readonly IEventRepository _eventRepository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ILogger<AssignVenueCommandHandler> _logger;

    public AssignVenueCommandHandler(
        IEventRepository eventRepository,
        IUnitOfWork unitOfWork,
        ILogger<AssignVenueCommandHandler> logger)
    {
    ...
    public async Task<Result<EventSessionDto>> Handle(AssignVenueCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Assigning venue {VenueId} to session {SessionId}", request.VenueId, request.SessionId);
        ...
        _logger.LogInformation("Venue {VenueId} assigned to session {SessionId}", request.VenueId, request.SessionId);
